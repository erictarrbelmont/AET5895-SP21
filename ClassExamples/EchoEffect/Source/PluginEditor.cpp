...
void EchoEffectAudioProcessorEditor::comboBoxChanged(ComboBox *comboBox){
    ...
    if (comboBox == &noteSelector){
        ...
        if (noteSelector.getSelectedId() == 2){
            audioProcessor.noteSelect = Echo::NoteSelection::HALF;
        }
        if (noteSelector.getSelectedId() == 3){
            audioProcessor.noteSelect = Echo::NoteSelection::QUARTER;
        }
        if (noteSelector.getSelectedId() == 4){
            audioProcessor.noteSelect = Echo::NoteSelection::EIGHTH;
        }
        if (noteSelector.getSelectedId() == 5){
            audioProcessor.noteSelect = Echo::NoteSelection::SIXTEENTH;
        }
    }
}
